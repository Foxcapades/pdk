...
class LongDeque : PrimitiveDeque<Long, LongArray> {
  ...
  private var container: LongArray
  ...
  constructor(capacity: Int = 0) {
    this.container = LongArray(capacity)
  }
  ...
  override fun pushTail(values: LongArray) {
  ...
  override fun pushTail(values: Collection<Long>) {
    ensureCapacity(size + values.size)
    for (v in values)
      pushTail(v)
  }
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline fun pushLast(values: LongDeque) = pushTail(values)
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline fun pushBack(values: LongDeque) = pushTail(values)
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline operator fun plusAssign(values: LongDeque) = pushTail(values)
   ...
   */
  operator fun set(index: Int, value: Long) = container.set(internalIndex(validExtInd(index)), value)
  ...
  override fun copy() = LongDeque(container.copyOf(), realHead)
   ...
   * copied.
   */
  fun copyInto(array: LongArray, offset: Int = 0) {
  ...
  override fun equals(other: Any?) = if (other is LongDeque) container.contentEquals(other.container) else false
