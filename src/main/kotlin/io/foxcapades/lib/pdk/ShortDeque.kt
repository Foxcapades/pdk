...
class ShortDeque : PrimitiveDeque<Short, ShortArray> {
  ...
  private var container: ShortArray
  ...
  constructor(capacity: Int = 0) {
    this.container = ShortArray(capacity)
  }
  ...
  override fun pushTail(values: ShortArray) {
  ...
  override fun pushTail(values: Collection<Short>) {
    ensureCapacity(size + values.size)
    for (v in values)
      pushTail(v)
  }
   ...
   */
  fun pushTail(values: ShortDeque) {
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline fun pushLast(values: ShortDeque) = pushTail(values)
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline fun pushBack(values: ShortDeque) = pushTail(values)
   ...
   * @param values Deque that will be pushed onto the back of this deque.
   */
  inline operator fun plusAssign(values: ShortDeque) = pushTail(values)
   ...
   */
  operator fun set(index: Int, value: Short) = container.set(internalIndex(validExtInd(index)), value)
  ...
  override fun copy() = ShortDeque(container.copyOf(), realHead)
  ...
  override fun equals(other: Any?) = if (other is ShortDeque) container.contentEquals(other.container) else false
